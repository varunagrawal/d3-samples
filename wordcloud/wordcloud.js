windowWidth = 1000; //window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
windowHeight = 1000; //window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
...
function getWordCounts(data) {
    ...
    var res = arr.filter(function(x) { return x.count > 2 && x.count <= 35; });
    return res;
}
...
var chartGroup = svg.append("g").attr("transform", "translate(" + svg_width/2 + "," + svg_height/2 + ")");
...
d3.json("mlatgt_posters.json", function(error, data){
    ...
    console.log("Adversarial", word_counts.filter(function(x) { return x.text == "adversarial"}));
    ...
    var wordcloud = d3.layout.cloud()
        ...
        .words(word_counts)
        .padding(2)
        .rotate(function() { return is_rotated * (~~(Math.random() * 2) * 90); })
